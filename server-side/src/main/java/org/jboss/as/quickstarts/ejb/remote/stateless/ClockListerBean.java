...
public class ClockListerBean implements ClockLister {
	...
	public void verifyOrdersLoading() {
		...
		for ( Order order : orders ) {
			System.out.println( "Got Order#customer: " + order.getCustomer().getId() );

			if ( order.getCustomer().getAddress() == null ) {
				throw new RuntimeException( "Failed assertions" );
			}

			final PersistentAttributeInterceptable interceptable = (PersistentAttributeInterceptable) order.getCustomer()
					.getAddress();
			...
			try {
				final Class<?> proxyLazinessInterceptor = Class.forName(
	...
	@Override
	@Transactional
	public void callSetIdOnAEnhancedProxy() {
		final Session session = em.unwrap( Session.class );

		StatementsCountChecker scc = new StatementsCountChecker( session );

		final Order order = session.createQuery( "select o from Order o whre o.id = :id", Order.class ).setParameter(
				"id",
				1 ).getSingleResult();
		scc.verifyIncrement( 1 );
